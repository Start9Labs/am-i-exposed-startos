...
- **Hard dependencies on `mempool` and `tor`.** `main.ts` reaches Mempool's `webui` over the LXC bridge for the app's `/api` proxy (and derives a public-facing URL for the "View on local mempool" link), and reads tor's container IP so the `tor-proxy` sidecar can route Chainalysis lookups through tor's SOCKS proxy. Neither dependency is a package.json dependency — Mempool's host id is resolved at runtime from its interface.
